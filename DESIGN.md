...
KillerOS presents each delegated task as a named child thread, not as an invisible turn-limited run. The main agent selects a generic or custom role for each child; KillerOS ships no package-local roles. The thread contract records its parent ID, child ID, display name, attempt, role, prompt, model, requested capability boundary, stable session ID and directory, trace, usage, and result state. Model and thinking inherit the selected role or active parent unless the dispatch sets them. The parent owns scope, waiting, inspection, steering, collection, resumption, and closure.
...
A child becomes done only when it returns usable final assistant text. Each child stops at the first default limit reached: 64 turns, 2,000,000 reported tokens, or 30 minutes. Dollar quotas remain opt-in, and every JSONL record still has an 8 MiB parser ceiling. Steering restarts the same session so the child keeps its prior conversation and remaining limits. KillerOS bounds retained trace, stderr, and returned text and spills a large JSONL line to temporary storage; retention does not stop the child or mark it `limited`. When closing or replacing a process, KillerOS waits up to 10 seconds for confirmed exit; an unconfirmed exit keeps the child session directory and records the cause. Embedding callers may adjust the turn, token, or wall-time limits and apply explicit output, trace, stderr, JSONL, or cost guards; each guard must name its cause and return partial work clearly. The parent still bounds task count, reader concurrency, role files, task input, and combined parent output. Explicit user interruptions and real child-process failures remain visible. The main agent chooses the generic or custom role for each child. Model and thinking inherit the selected role or active parent unless the dispatch sets them; lifecycle controls do not expand role permissions.
